...
def test_meus_pedidos_sem_filtros(headers):
    ...
    """
    resp = requests.get(
        f"{BASE_URL}/api/trafego/historico/meus-pedidos", headers=headers
    )
...
def test_meus_pedidos_filtro_service_id(headers):
    ...
    """
    resp = requests.get(
        f"{BASE_URL}/api/trafego/historico/meus-pedidos?service_id=171", headers=headers
    )
    assert resp.status_code == 200
    data = resp.json()
    for pedido in data["data"]:
...
def test_status_pedido(headers):
    ...
    resp = requests.get(
        f"{BASE_URL}/api/trafego/pedidos/{order_id}/status", headers=headers
    )
